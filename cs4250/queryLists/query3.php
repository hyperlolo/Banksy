...
<head>
    <title>Average Sum In All Accounts|Query List|Banksy</title>
</head>
...
<div id="canWithdraw">
    ...
    <span>For any questions or concerns please contact us <a href="mailto:kgill15@csustan.edu?&cc=ggarciadeleon@csustan.edu;cguptail@csustan.edu&subject=Average Sum In All Accounts%20Query%20List" style="color: inherit;"target="_blank">HERE</a></span>
    </footer>

    <div id="Canwithdraw" style="z-index: 9000;">
		<span>Average Sum In All Accounts</span>
    </div>
    
    <div id="Query__SELECT___FROM__canWithd">
        <span>SELECT AVG(balance), SUM(balance), COUNT(memberSSN) FROM `BankAccount`;</span><br><br>
        <p>A manager may want to see how much money on average is being held by members at all banks in the network.</p>
        <hr>
        <p>Finds out the average balance of all the accounts of all the member in the BankAccount table, and the sum of all of the balances in BankAccount combined.</p>
    </div>
    ...
    <section id="results" style="top:485px";>
        ...
        $sql = "SELECT AVG(balance), SUM(balance), COUNT(memberSSN) FROM `BankAccount`;";
