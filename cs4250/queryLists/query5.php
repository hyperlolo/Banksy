...
<head>
    <title>Low Member States|Query List|Banksy</title>
</head>
...
<div id="canWithdraw">
    ...
    <span>For any questions or concerns please contact us <a href="mailto:kgill15@csustan.edu?&cc=ggarciadeleon@csustan.edu;cguptail@csustan.edu&subject=Low Member States%20Query%20List" style="color: inherit;"target="_blank">HERE</a></span>
    </footer>

    <div id="Canwithdraw" style="z-index: 9000;">
		<span>Low Member States</span>
    </div>
    
    <div id="Query__SELECT___FROM__canWithd">
        <span>SELECT DISTINCT state FROM Member GROUP by state HAVING COUNT(state) < 2 ORDER BY state;</span><br><br>
        <p>A manager may want to focus on the states where they have a low number of members at a bank branch.</p>
        <hr>
        <p>Finds out any states which have less than 2 users, this information can be used to inform a bank on which states they should focus their marketing campaign on.</p>
    </div>


    <section id="results" style="top:485px";>
        <main role="main">
        <article>
        ...
        $sql = "SELECT DISTINCT state FROM Member GROUP by state HAVING COUNT(state) < 2 ORDER BY state;";
